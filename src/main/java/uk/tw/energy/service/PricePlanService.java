...
public class PricePlanService {
	...
	private Function calculareReadingList;
	...
	public BigDecimal readPreviousWeekConsumptionByUserId(String smartMetId) {
		List<ElectricityReading> reading = meterReadingService.getReadings(smartMetId).orElseThrow(
				() -> new ConsumptionServicesException(ConsumptionServicesException.PLAN_NOT_FOUNDED, smartMetId));
		PricePlan pricePlan = this.getPricePlanById(smartMetId);
		return this.calculateCost(reading, pricePlan);
	}

	private PricePlan getPricePlanById(String smartMetId) {
		//@FIXME find real the plan
		return new PricePlan("Cheap Plan", "Supplier1 for " + smartMetId, new BigDecimal(0.2F), new ArrayList<>());
	}
